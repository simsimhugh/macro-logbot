...
### task-AGENT-021 — control-flow / infinite-loop 패턴 prompt 강화 (E008 fail 대응)
- **출처**: PR #54 본문 (N=3 baseline 의 유일한 fail case 분석).
- **scope**: E008 (spawn_food 의 `while True` 에서 return 누락) 가 본 sprint 의 유일 fail case. agent 가 TimeoutError stderr 만 echo 하고 control-flow 분석 못함. PR #50 의 system prompt 강화 (semantic 오류) 가 본 case 에는 부족. system prompt 에 "TimeoutError 시 control-flow 분석 (loop 종료 조건 / return 누락 / break 누락) 강제" 추가. **regression 가드 — 본 prompt 추가가 단순 case (E001~E007/E009/E010) 의 1-A 점수 하락시키지 않는지 N=3 sample 재측정 필수**.
- **suggested branch**: `feat/system-prompt-control-flow`
- **reviewer scope**: 일반
- **size estimate**: prompt diff ~20 lines + tests ~40 lines + N=3 sample 재측정 effect 검증
- **priority**: medium — 본 sprint 의 유일 fail case 직접 close (90% → 95%+ 자율해결률 가능)
