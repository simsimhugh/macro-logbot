...
### task-EVAL-006 — N=10 baseline 보고서 §3 "F2 해소 ✅" 결론 정정
- **출처**: PR #52 본문 (workdir mode fix 발견 후 N=10 보고서의 false positive 정정 필요).
- **상태**: **PR #54 으로 closed**. `docs/showcase/baseline-2026-05-20-N10.md` header 에 DEPRECATED 표기 + 새 baseline `docs/showcase/baseline-2026-05-20-N3-after-pr53.md` 신규 (PR #50/52/53 적용 후 측정, 자율해결률 90%, full 30%, avg total 73.27/100).

### task-AGENT-021 — control-flow / infinite-loop 패턴 prompt 강화 (E008 fail 대응)
- **출처**: PR #54 본문 (N=3 baseline 의 유일한 fail case 분석).
- **scope**: E008 (spawn_food 의 `while True` 에서 return 누락) 가 본 sprint 의 유일 fail case. agent 가 TimeoutError stderr 만 echo 하고 control-flow 분석 못함. PR #50 의 system prompt 강화 (semantic 오류) 가 본 case 에는 부족. system prompt 에 "TimeoutError 시 control-flow 분석 (loop 종료 조건 / return 누락 / break 누락) 강제" 추가.
