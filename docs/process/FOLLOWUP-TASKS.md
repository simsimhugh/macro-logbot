...
- **출처**: baseline N=3 측정 2026-05-20 (F2). 30/30 case 가 agent tool 호출 실패 — workspace boundary 거절 (`/tmp/poc-E001-xxx/snake.py` 가 backend `/app` workspace 밖).
- **scope**: `src/macro_logbot/tools/` 의 `read_file` / `grep_codebase` / `list_directory` 가 workspace 정책 확장. PoC 측정 시 `MACRO_LOGBOT_POC_WORKSPACE_ALLOWED=/tmp/poc-*` env 로 추가 path 허용. 또는 inject.py 가 backend workspace 안 (`/app/poc-cases/<case_id>/`) 으로 코드 copy 후 trigger. 본 task 로 baseline 측정의 진짜 의미 가 회복 (현재는 keyword 우연 match).
- **suggested branch**: `feat/tool-workspace-poc-allow`
- **reviewer scope**: 일반 + security-reviewer 중점 (path traversal risk + allowlist)
- **size estimate**: src ~60 lines + tests ~80 lines + 재측정 baseline
