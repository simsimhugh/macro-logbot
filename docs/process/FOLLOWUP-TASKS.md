...
- **scope**: `--models gemini/...,openai/...,anthropic/...,groq/...` 다중 swap + per-model 결과 분리 저장 + comparison.md 의 모델 매트릭스 (`docs/process/04-PoC-운영가이드.md` §7.4 표 형식). 약한 LLM 강화 사이클의 baseline 필수.
...
- **scope**: `poc/scripts/evaluate.py` 가 현재 `POST /agent/analyze` 단일 호출. spec `docs/design/02-설계문서.md` §9.4 + `docs/process/04-PoC-운영가이드.md` §6.3 는 `POST /events` → session_id polling → `GET /sessions/<id>/report` 흐름. **task-MVP-004 (session 통합) 완료 후** evaluate.py 마이그레이션, 또는 단기적으로 04-PoC-운영가이드 §6.3 endpoint 명세를 현행 MVP 와 일치하게 annotation.
...
### task-AGENT-010 — backend agent loop context 모니터링 + auto-truncate
- **출처**: baseline N=3 측정 2026-05-20 (F1, `docs/showcase/baseline-2026-05-20.md`). LM Studio Gemma 3 12B 의 context window 초과로 N1 첫 4 case `HTTP 500 — OpenAIError 400: Context size has been exceeded`.
- **scope**: agent loop 가 매 turn 후 누적 token 추정 + LM 모델별 context 한계 (env `MACRO_LOGBOT_MODEL_CONTEXT_LIMIT` 또는 모델 카탈로그) 와 비교. 초과 예측 시 자동 truncate (오래된 tool 응답 / reasoning 우선 제거, system prompt + 최신 stderr 보존). 또는 small reasoning summary 로 압축. 실측 효과 ablation 측정.
- **suggested branch**: `feat/agent-context-management`
- **reviewer scope**: 일반 + architect 중점 (context 정책 결정)
- **size estimate**: src ~120 lines + tests ~80 lines + ablation 측정
- **priority**: **high** — Local LLM (≤ 12B Q4) baseline 측정 차단

### task-TOOL-001 — backend tool workspace 정책 — PoC `/tmp/poc-*` 허용
- **출처**: baseline N=3 측정 2026-05-20 (F2). 30/30 case 가 agent tool 호출 실패 — workspace boundary 거절 (`/tmp/poc-E001-xxx/snake.py` 가 backend `/app` workspace 밖). architect / security reviewer 의 PR #41 추가 권고 반영.
- **scope** (architect 권고 = inject.py copy 우선, env allowlist 회피):
  - **권장 (A)**: `inject.py` 가 backend workspace 안 (`/app/poc-cases/<case_id>/`) 으로 코드 copy 후 trigger. backend tool 정책 변경 없음 — production parity 유지. **architect WARN-2 회피**.
  - **차선 (B, env allowlist)**: `MACRO_LOGBOT_POC_WORKSPACE_ALLOWED=/tmp/poc-*` env 로 추가 path 허용. **단 security 4 점 필수** (security-reviewer INFO-1):
    1. **literal prefix** (regex 금지, glob 만 — traversal injection 차단)
    2. **`O_NOFOLLOW`** (symlink 거부)
    3. **secret blocklist** (env files, key files, `~/.ssh/*`, `~/.aws/*` 절대 reject)
    4. **`env=poc` gate** (`MACRO_LOGBOT_ENV=poc` 환경에서만 활성. production 환경에서 fail-closed)
- **suggested branch**: `feat/tool-workspace-poc-allow`
- **reviewer scope**: 일반 + security-reviewer 중점 (path traversal risk + allowlist + production parity)
- **size estimate**: 옵션 A: src ~30 lines + tests ~50 lines. 옵션 B: src ~80 lines + tests ~120 lines (4 보안 layer 포함)
- **priority**: **high** — baseline 측정의 가치 회복 (현재 자율분석률 50~80% 가 false positive)

### task-AGENT-011 — crystallize 단계 structured Report 변환 강화
- **출처**: baseline N=3 측정 2026-05-20 (F3). 30/30 case 가 `report.location == None`. crystallize 단계가 LLM 자연어 답을 그대로 `root_cause` / `fix_hint` 에 박음, structured 변환 없음.
- **scope**: `src/macro_logbot/agent/nodes/crystallize.py` (또는 해당 모듈) 의 LLM 호출 prompt 강화 — 명시적 JSON schema 강제. `Report` schema 미준수 시 retry 1회. 또는 structured output API (litellm response_format) 사용. tool 사용 못 한 경우라도 stderr 의 traceback 에서 file:line 추출 fallback 추가.
- **suggested branch**: `feat/crystallize-structured-output`
- **reviewer scope**: 일반 + architect 중점
- **size estimate**: src ~100 lines + tests ~120 lines
- **priority**: medium — task-TOOL-001 fix 후 효과 측정 가능

### task-EVAL-001 — evaluate.py spec 정합 (4-channel scoring + temperature/seed + session cumulative)
- **출처**: baseline N=3 측정 architect agent 점검 결과. spec §10.1 / §10.4 / §10.6 위반:
  - 1-A scoring 가중치 (`evaluate.py:113-115`: 0.4·file + 0.3·line + 0.3·kw) 가 spec §10.1 의 25%×4 channel 과 다른 공식.
  - `evaluate.py:58-61` payload 에 `temperature`/`seed` 미포함 → LiteLLM default → N=3 variance.
  - payload 에 `session_id` 미포함 → spec §10.6 cumulative mode 작동 안 함.
  - DEFAULT_MODEL = gemini (spec §7.1 baseline 은 Groq Llama).
- **scope**: payload 에 `temperature=0`, `seed=42`, optional `session_id` 추가. total scoring 을 `0.25·(1-A) + 0.25·(1-B) + 0.25·(2-A) + 0.25·(2-B)` 로 변경 (4 channel 가중치 spec 정합). DEFAULT_MODEL 결정은 사용자 결정 사항 (baseline vs 권장).
- **suggested branch**: `feat/evaluate-spec-compliance`
- **reviewer scope**: 일반 + architect 중점
- **size estimate**: src ~80 lines + tests ~80 lines + spec 갱신
- **priority**: **high** — 재측정 전 선결

### task-KB-004 — `ArchivedCase` tier 정책 구현 (draft / verified / verified-master)
- **출처**: 사용자 critical 지적 (2026-05-20 02:30). `_kb_auto_archive` (`src/macro_logbot/app.py:311-324`) 가 분석 결과를 검증 없이 KB add — self-bootstrapping risk. spec `docs/design/02-설계문서.md:231,252,253,261` 에 정책 정의됨 (`source: poc | production | verified-master`), 코드 미구현. `task-KB-002` line 259 가 잔여 명시: "verifier 승격 hook → 별도".
- **scope**:
  - `ArchivedCase` 에 `tier: Literal["draft","verified","verified-master"]` 필드 추가 (또는 `source` field 활용).
  - `_kb_auto_archive` 가 새 case 를 `tier="draft"` 로 add.
  - KB 검색 (`agent/nodes/...` 또는 tool) 시 `tier in ["verified","verified-master"]` 만 hit. `draft` 는 audit 전용.
  - **verifier 승격 hook**: spec §10 의 "PoC 매트릭스 정답 일치 시 자동 verified-master 승격" — evaluate.py 가 case score ≥ threshold 시 KB API 호출하여 tier 승격. 또는 사람 admin 명령.
  - 마이그레이션: 기존 `archived_cases` row 모두 `tier="draft"` 로 backfill.
- **suggested branch**: `feat/kb-archive-tier-policy`
- **reviewer scope**: 일반 + security-reviewer 중점 (self-bootstrapping risk)
- **size estimate**: src ~150 lines + tests ~120 lines + 마이그레이션 SQL
- **priority**: **high** — 운영 진입 gate (사내 KB 누적 시 garbage 확산 차단)
- **관련**: [[project-macro-logbot-status]] 의 KB design flaw, `task-KB-001` 벡터 임베딩, `task-KB-002-x` write 품질.

### task-TOOL-002 — backend tool caller 의 `O_NOFOLLOW` 적용 (TOCTOU race 해소)
- **출처**: PR #43 architect WARN-2 + code-reviewer BLOCK-2 + security HIGH-1. `builtin.py` 의 `_safe_resolve` 가 `Path.is_symlink()` 검사 시점과 `read_file` / `grep_codebase` / `list_directory` 의 실제 open 사이에 race condition (TOCTOU). 검사 후 attacker 가 symlink 교체 가능.
- **scope**:
  - `read_file` / `grep_codebase` / `list_directory` 3 caller 를 refactor — `os.open(path, os.O_RDONLY | os.O_NOFOLLOW)` 사용 (또는 fd-based io).
  - 또는 `_safe_resolve` 가 fd 를 반환하도록 변경 + caller 가 fd 사용 — race-free.
  - 신규 test — TOCTOU race regression (검사 후 symlink replace 시도 → open fail 확인).
- **suggested branch**: `feat/tool-fd-based-io-toctou-fix`
- **reviewer scope**: 일반 + security-reviewer 중점 (TOCTOU 해소 검증)
- **size estimate**: src ~100 lines (3 caller refactor) + tests ~80 lines
- **priority**: medium — PR #43 의 `_safe_resolve` 가 stat-time 검사만 (docstring 명시) — PoC 측정 시 attacker 없으므로 risk 낮지만 production 진입 전 필수.
- **관련**: PR #43 의 docstring TOCTOU 명시, security HIGH-1 (issuecomment-4492764856), architect WARN-2 (issuecomment-4492769690), code-reviewer BLOCK-2 (issuecomment-4492768740).

### task-SEC-015 — crystallize prompt 3중 시크릿 echo 차단
- **출처**: PR #46 architect MED WARN-4 + code-reviewer WARN-1.
- **scope**: `_CRYSTALLIZE_SYSTEM_PROMPT` + user content (last_assistant_content) + corrective retry prompt 가 시크릿을 3중 echo 가능. crystallize prompt 에 시크릿 마스킹 또는 content 길이 cap + PII redact 적용. task-SEC-011 의 하위 항목.
- **suggested branch**: `chore/sec-crystallize-secret-echo`
- **reviewer scope**: 일반 + security-reviewer 중점
- **priority**: medium — task-SEC-011 와 묶음 가능

### task-AGENT-012 — crystallize traceback fallback user-code frame filter (stdlib 제외)
- **출처**: PR #46 architect MED WARN-5.
- **scope**: `_location_from_traceback` 가 stdlib / site-packages 프레임을 user-code 로 잘못 보고 가능. `Path(m.group(1))` 이 `/usr/lib/` 또는 `.venv/` 포함 시 skip, user-code 마지막 프레임만 반환.
- **suggested branch**: `fix/traceback-usercode-frame-filter`
- **reviewer scope**: 일반
- **priority**: low — 분석 품질 향상

### task-AGENT-013 — crystallize LLM cost cap (max_tokens)
- **출처**: PR #46 security MED-2.
- **scope**: crystallize LLM 호출에 `max_tokens` cap 미설정 → 과금 위험. `_generation_kwargs` 에 없을 때 기본값 (예: 512) 주입.
- **suggested branch**: `chore/crystallize-max-tokens-cap`
- **reviewer scope**: 일반
- **priority**: low — PoC 환경 위험 낮음, production 진입 전 필수

### task-AGENT-014 — structured-JSON file path sanitization (`Path().name` 적용)
- **출처**: PR #46 security MED-1.
- **scope**: `_crystallize_report_node` 의 Location 생성 시 `loc_data.get("file")` 를 그대로 사용 — LLM 이 절대경로 주입 가능. `Path(str(loc_data.get("file") or "")).name` 으로 basename 만 사용.
- **suggested branch**: `fix/crystallize-location-path-sanitize`
- **reviewer scope**: 일반 + security-reviewer
- **priority**: medium — LLM 프롬프트 인젝션 경로

### task-TEST-003 — test_agent_nodes.py crystallize 테스트 gateway mock 갱신
- **출처**: PR #46 code-reviewer WARN (CR INFO).
- **scope**: `test_agent_nodes.py` 의 5개 crystallize 테스트가 `LLMGateway.__new__()` (complete 미정의) 를 사용 — LLM 호출 실패 fallback path 만 검증. `AsyncMock` 기반 gateway 로 교체하여 structured JSON 경로도 커버.
- **suggested branch**: `test/crystallize-gateway-mock-upgrade`
- **reviewer scope**: 일반
- **priority**: low — 기능 정확성은 test_crystallize_structured.py 가 커버

### task-EVAL-002 — evaluate.py 1-A 함수를 30점 binary 정의와 정합
- **출처**: PR #49 architect WARN-2 + code-reviewer HIGH-1 + test-engineer implementation gap.
- **scope**: 현재 `poc/scripts/evaluate.py` 의 `score_1a()` 가 이전 4-channel 25%×4 의 1-A heuristic (`0.4·file + 0.3·line + 0.3·kw_ratio`) 그대로. 새 30점 기준 (도구 호출 5 + 리포트 생성 5 + file:line 10 + tool 적절 10) 에 정합되어야:
  - `iter > 1` (도구 호출 성공) 5점 binary
  - `report.location is not None` (structured Report) 5점 binary
  - file:line heuristic 10점 (현재 file_match + line_match 변환)
  - `expected_tool_calls` 일치 10점 (현재 LLM judge 의 2-A 가 처리 — 자동 측정으로 옮김)
  - `write_comparison()` 헤더 "spec §10.1 4단계 가중 합산" → "spec §10.1 자동 30 + Claude judge 70" 으로 갱신.
- **suggested branch**: `feat/evaluate-30-binary-rubric`
- **reviewer scope**: 일반 + test-engineer 중점 (자동 채점 정확성)
- **size estimate**: src ~80 lines (score_1a 재작성 + write_comparison) + tests ~100 lines
- **priority**: **high** — PR #49 머지 시 즉시 spec ↔ 코드 drift 발생. 다음 측정 전 선결.

### task-AGENT-018 — crystallize prompt 형식을 system prompt 와 통일
- **출처**: PR #50 architect WARN.
- **scope**: system prompt (`app.py:240-255`) 의 답안 형식 ("### root_cause / ### fix_hint", "X 라인 A→B") 와 crystallize prompt (`agent/core.py:205-207`) 의 JSON schema 가 불일치. agent loop 가 자연어 답을 자유 형식으로 생성한 후 crystallize 가 JSON 으로 강제 변환. 두 단계 형식 통일 (system prompt 도 JSON 직접 강제 또는 crystallize 가 system prompt 의 ### root_cause/### fix_hint marker 추출).
- **suggested branch**: `feat/system-crystallize-format-unify`
- **reviewer scope**: 일반 + architect 중점
- **size estimate**: src ~60 lines + tests ~80 lines
- **priority**: medium

### task-AGENT-019 — trivial/unanalyzable case escape ("Unknown: <사유>")
- **출처**: PR #50 architect WARN (가장 시급).
- **scope**: PR #50 의 보강 commit 이 "분석 불가 시 `Unknown: <사유>` 로 명시" 한 줄 추가했지만 강제 mechanism 없음. agent 가 "최소 2회 tool 호출" 의무 + "추측 금지" 강제 결과 trivial error (e.g., 빈 stderr, INFO 메시지) 입력 시 hallucinate 또는 timeout 발생 가능. crystallize 단계에서 LLM 답안의 confidence < 0.3 또는 root_cause 가 traceback frame 인용 없는 경우 자동 "Unknown" 처리.
- **suggested branch**: `feat/crystallize-unknown-escape`
- **reviewer scope**: 일반
- **size estimate**: src ~40 lines + tests ~80 lines
- **priority**: **high** — hallucinate risk 차단

### task-AGENT-020 — 사내 환경 noise 패턴 카탈로그화
- **출처**: PR #50 architect WARN.
- **scope**: PR #50 의 noise filter 6 패턴 (ALSA, pygame, GPU, locale, Deprecation, urllib3) 는 사외 PoC E001-E010 fixture 기준. 사내 환경 noise (Windows COM error, .NET CLR exception, MFC dialog, 사내 protocol 라이브러리 등) 추가 필요. 사내 LLM 측정 시점에 stderr sample 분석 후 카탈로그 확장.
- **suggested branch**: `feat/system-prompt-internal-noise`
- **reviewer scope**: 일반
- **size estimate**: prompt diff ~30 lines + 사내 sample 분석 보고서
- **priority**: low — 사내 측정 시점에 처리

### task-AGENT-015/016/017 — PR #50 system prompt 강화 entry 등록 누락
- **출처**: PR #51 code-reviewer MED.
- **scope**: PR #50 가 task-AGENT-015 (fix_hint 구체성) / 016 (semantic 오류 분석) / 017 (stderr noise filter) 를 한 commit 으로 처리했으나 본 FOLLOWUP-TASKS.md 에 entry 등록 안 함 — 본 closed 행에 retroactive 등록 (process consistency).
- **상태**: PR #50 머지 (b61ce18) 로 closed. 별도 작업 없음.

### ~~task-EVAL-006~~ — N=10 baseline 보고서 §3 "F2 해소 ✅" 결론 정정 ✅ **PR #54 머지**
- **출처**: PR #52 본문 (workdir mode fix 발견 후 N=10 보고서의 false positive 정정 필요).
- **처리 PR**: PR #54 — `docs/showcase/baseline-2026-05-20-N10.md` header DEPRECATED 표기 + 새 baseline `docs/showcase/baseline-2026-05-20-N3-after-pr53.md` 신규 (PR #50/52/53 적용 후 측정, 자율해결률 90%, full 30%, avg total 73.27/100).

### ~~task-AGENT-021~~ — control-flow / infinite-loop 패턴 prompt 강화 ❌ **CLOSED (regression)**
- **출처**: PR #54 본문 (N=3 baseline 의 유일한 fail case 분석).
- **결과**: 시도 후 close — system prompt §5 에 TimeoutError 가이드 추가했으나 N=3 측정에서 **avg 0.801 → 0.453 (-0.348) 큰 regression**. E001 N2 의 40-message 무한 loop 분석 결과 **진짜 root cause = prompt 가 아닌 agent termination 약함** (같은 tool+args 반복 시 break 부재). 본 task close + **task-AGENT-022 (진짜 fix)** 로 대체. branch + push 정리 완료.

### task-AGENT-022 — agent termination guard (same-tool-args repeat detection)
- **출처**: PR #55 (task-AGENT-021 close 후 진짜 root cause 발견).
- **scope**: PR #54 N=3 baseline 후 E001 N2 의 40-message 무한 loop 사례 분석 결과, agent 가 같은 (tool_name, args) 를 연속 반복하다 max_iters 도달 → final answer 없음 → 1-A=0.000 패턴 발견. `_route_after_llm` 에 same-tool-args repeat detection 추가 — 최근 N=3 회 assistant tool_call 이 모두 같은 signature 면 crystallize_report 로 break.
- **suggested branch**: `feat/agent-termination-guard`
- **reviewer scope**: 일반
- **size estimate**: src ~25 lines + tests ~150 lines + N=3 sample 재측정 effect 검증 (별도 PR)
- **priority**: **high** — 본 sprint baseline 의 hidden bug (E008 단독이 아니라 모든 case 의 잠재 risk)

### task-OPS-002 — run-onprem-baseline.sh robustness 보강
- **출처**: PR #54 architect MED + test-engineer MED + code-reviewer MED 다수.
- **scope**:
  - (a) session DB SQL 의 `LIMIT N*10` 가 이전 측정 session 섞일 수 있음 → 측정 시작 시각 `STARTED_AT` 캡쳐 후 `WHERE created_at >= STARTED_AT` 또는 `session_id IN (...)` 으로 강제
  - (b) tool result error detection `"error" in content[:50]` substring → JSON parse 후 `obj.get("ok") is False` 또는 `"error" in obj` key 기반 판정
  - (c) `MACRO_LOGBOT_BACKEND_CONTAINER` env override 를 `.env.bak` template 에 추가 + 자동 검출 (`docker compose ps -q backend`)
  - (d) `run.log` 분리 (run-N{1..N}.log) + `tee` 로 화면 + log 동시 출력 (진행상황 가시성)
  - (e) `RATE_LIMIT_COOLDOWN` env override 추가 (사내 LLM 의 rate limit 대응)
  - (f) docker exec invariant check 실패 시 측정 결과까지 abort — `|| true` 또는 별도 `set +e` 블록으로 격리
  - (g) N / CASES arg 정수 + format 검증 (`[[ "$N" =~ ^[0-9]+$ ]]`)
- **suggested branch**: `fix/onprem-baseline-script-robustness`
- **reviewer scope**: 일반 + test-engineer (bats 테스트 추가)
- **size estimate**: script ~30 lines + bats tests ~80 lines
- **priority**: **high** — 사내 측정 첫 시도 신뢰성 직접 영향

### task-SEC-016 — sed redact 가이드 강화 + Python 스크립트 분리
- **출처**: PR #54 security MED + code-reviewer LOW.
- **scope**:
  - (a) §9.4 의 sed redact 가 JWT/bearer/api-key/k8s/사번/email 패턴 누락 → 추가
  - (b) `sed -i` GNU/BSD 비호환 → Python 스크립트 `poc/scripts/redact_onprem.py` 분리
  - (c) tar.gz 만들기 전 grep 검증 의무 step 추가 (`grep -rE '(Bearer |eyJ|sk-|cluster\.local)' "$ROOT/reports"` 결과 있으면 abort)
  - (d) `.env.bak` source 시 secret 이 child process env 로 export — `# WARNING` 주석 + Bash trap EXIT 시 unset
  - (e) `run.log` 에서도 secret pattern redact (best-effort sed)
- **suggested branch**: `feat/onprem-redact-strict`
- **reviewer scope**: security-reviewer + 일반
- **size estimate**: src (redact_onprem.py) ~80 lines + docs ~30 lines + tests ~60 lines
- **priority**: **high** — 사내 측정 raw JSON 의 사외 전달 안전성 직접 영향

### task-EVAL-008 — 외부 LLM judge cross-validation (self-judge calibration)
- **출처**: PR #54 code-reviewer Open Question.
- **scope**: 현 baseline 보고서의 4-channel total 채점이 본 PR 작성 Claude main session 의 self-judge — N=10 false positive 와 동일한 self-validation 위험. 외부 judge (Gemini Flash, GPT-4o, Claude Haiku 등) 도 같은 case 채점 후 매트릭스 비교 → 본인 채점의 systematic bias 검출. PR #51 의 `--judge` flag 활용 + `claude_judge.py` 확장.
- **suggested branch**: `feat/external-judge-calibration`
- **reviewer scope**: 일반
- **size estimate**: src ~50 lines + 측정 결과 분석 ~100 lines + 보고서 추가
- **priority**: medium — 본 baseline 의 신뢰도 calibration

### task-JUDGE-001 — claude_judge.judge_tool_appropriateness 의 tool result error 채점 점검
- **출처**: PR #52 architect HIGH.
- **scope**: PR #51 §2.2 의 2-A (tool appropriateness) 채점이 모든 case 에서 full 1.0 으로 나온 것은 judge prompt 가 tool 호출 행위만 평가하고 result (error 여부) 는 평가 안 했을 가능성. `src/macro_logbot/.../claude_judge.py` 의 `judge_tool_appropriateness` prompt 점검 — tool result 가 모두 error 인 경우 partial 미만 채점하도록. 본 점검 후 필요 시 prompt 수정 + regression test 추가.
- **suggested branch**: `fix/claude-judge-tool-result-error`
- **reviewer scope**: 일반
- **size estimate**: prompt 분석 + (필요 시) src ~30 lines + tests ~60 lines
- **priority**: **high** — N=10 보고서의 2-A 1.0 false positive 의 root cause 일 가능성

### task-POC-007 — trigger.py 의 `__pycache__` 가 host umask 로 0o700 생성
- **출처**: PR #52 architect MED.
- **scope**: `poc/scripts/trigger.py` 의 `subprocess.run(cwd=workdir)` 가 Python interpreter 를 host uid 로 돌릴 때 `workdir/__pycache__/` 생성 → host umask 0o077 환경에서는 0o700 mode → backend container 가 list_directory 에서 partial blackout 가능. 본 sprint 의 read_file 은 snake.py 만 대상이라 현재는 무증상이지만, multi-file injection 또는 향후 list_directory 광범위 사용 시 재발. **Fix**: `subprocess.run(env={**env, "PYTHONDONTWRITEBYTECODE": "1"})` 또는 trigger 직후 workdir 안 모든 dir 의 chmod 0o755 적용.
- **suggested branch**: `fix/poc-trigger-pycache-mode`
- **reviewer scope**: 일반
- **size estimate**: src ~5 lines + tests ~30 lines
- **priority**: medium — 무증상이지만 재발 가능성 명백

### task-TEST-session-flaky — test_update_refreshes_updated_at 고정 datetime 수정
- **출처**: PR #52 test-engineer 발견 (PR 도입 전부터 pre-existing flaky).
- **scope**: `tests/test_session_store.py::test_update_refreshes_updated_at` 의 `_now` monkeypatch 가 `datetime(2026, 5, 19, 10, 0)` 고정값. 실행 시각이 본 고정값보다 미래면 항상 fail. **Fix**: 고정 datetime 을 `datetime.now(UTC) + timedelta(seconds=1)` 패턴으로 변경 + monkeypatch 가 시간 흐름을 직접 control.
- **suggested branch**: `fix/test-session-store-flaky`
- **reviewer scope**: 일반
- **size estimate**: tests ~10 lines
- **priority**: medium — 본 PR #52 회귀 분석 시 매번 노이즈

### task-INFRA-001 — inject.py 의 두 chmod 단일화 검토 (cleanup)
- **출처**: PR #52 code-reviewer LOW.
- **scope**: PR #52 가 inject.py 의 snake.py + case.yaml 둘 다 chmod 0o644 적용. workdir.chmod(0o755) 만으로도 폴더 안 파일 read 가능하므로 두 file chmod 는 후방 안전망 (defense in depth). 실측으로 두 chmod 제거해도 read 가능한지 검증 후 simplify 검토.
- **suggested branch**: `chore/inject-chmod-simplify`
- **reviewer scope**: 일반
- **size estimate**: src ~5 lines (제거) + tests ~10 lines (verify)
- **priority**: low — optional cleanup, 현재 동작 안전

### task-EVAL-005 — evaluate.py fail-fast guard (tool error sentinel 검출)
- **출처**: PR #51 false positive 의 silent pass root cause.
- **상태**: **PR #53 으로 closed** (poc/scripts/evaluate.py 의 TOOL_ERROR_SENTINELS + infra_error flag 추가 + tests/test_poc_evaluate.py 의 4 신규 test).

### task-EVAL-007 — AgentAnalyzeResponse 에 `tool_call_summary` 필드 추가
- **출처**: PR #53 architect HIGH.
- **scope**: `src/macro_logbot/app.py` 의 `AgentAnalyzeResponse` 가 현재 tool_calls 미노출 → evaluate.py 의 fail-fast 가 analysis_text substring 검사만 가능 (agent 가 traceback echo 안 하고 "분석 못함" 만 답하면 false negative). `AgentAnalyzeResponse` 에 `tool_call_summary: {tool_calls_count: int, tool_errors_count: int}` 추가 + evaluate.py 가 본 필드로 §7.5 invariant #2 ("tool_calls 0 회 또는 모두 error") 자동 검증.
- **suggested branch**: `feat/agent-analyze-tool-call-summary`
- **reviewer scope**: 일반
- **size estimate**: src (backend) ~30 lines + src (evaluate.py) ~15 lines + tests ~60 lines
- **priority**: **high** — §7.5 invariant 의 자동화 완성, 본 PR 의 architectural gap 직접 close

### task-EVAL-008 — TOOL_ERROR_SENTINELS extension point + permission family 확장
- **출처**: PR #53 architect LOW + security-reviewer follow-up.
- **scope**: 현재 hardcoded tuple. CLI flag (`--extra-sentinels`) 또는 env (`MACRO_LOGBOT_POC_EXTRA_SENTINELS`) 으로 확장 가능 + permission family (`EACCES`, `Operation not permitted`, `denied to open`) 추가. 사내 LLM 측정 시점 또는 다른 backend 환경에서 새 sentinel 발견 시 src 수정 + PR 없이 대응.
- **suggested branch**: `feat/eval-sentinel-extension`
- **reviewer scope**: 일반
- **size estimate**: src ~15 lines + tests ~30 lines
- **priority**: low

### task-DOC-001 — 보고서 template 에 §7.5 disclaim frontmatter 필수화
- **출처**: PR #53 architect MED.
- **scope**: `docs/showcase/*.md` 보고서가 §7.6 체크리스트를 self-enforce 하도록 template 신규 (`docs/showcase/_template.md`) — disclaim frontmatter 필수 (tool success rate, infra_error case 목록, max_iters case 목록, median run 선택). reviewer 가 frontmatter 누락 시 즉시 차단. PR #51 type 의 망각 재발 방지.
- **suggested branch**: `docs/showcase-template-disclaim`
- **reviewer scope**: 일반
- **size estimate**: docs ~50 lines
- **priority**: medium — PR #51 self-enforce 강제력 확보

### task-EVAL-009 — max_iters 종료 case 의 신뢰도 invariant 추가
- **출처**: PR #53 architect MED.
- **scope**: §7.5 의 5번째 invariant 로 "terminated_reason='max_iters' case 는 final answer 없이 종료 → 분모 제외" 명문화. 본 PR 의 4 invariant 가 PR #51 type false positive 는 cover 하지만 max_iters conflate 는 별개 risk.
- **suggested branch**: `docs/eval-max-iters-invariant`
- **reviewer scope**: 일반
- **size estimate**: docs ~15 lines
- **priority**: medium
