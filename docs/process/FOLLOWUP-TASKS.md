...
- **scope**: `docs/showcase/baseline-2026-05-20-N10.md` 의 §3 표 "F2 해소 ✅" + §2.2 의 2-A 1.0 × 10 case 행 + §3 의 "자율분석 80%" 결론을 정정. 본 측정 시점의 backend 가 PR #52 적용 전이라 모든 tool 호출이 Permission denied 였음 (session DB 검증 완료). 본 PR #53 의 §7.5 invariant 적용 시 본 측정은 "tool result success rate 0% — 신뢰 어려운 측정" 으로 disclaim. **재측정 N=3 데이터 합쳐서 보고서 작성 권고** (별도 PR 예정).
- **suggested branch**: `docs/n10-report-correction-and-rerun`
- **reviewer scope**: 일반 + verifier (수치 정정 정확성 재검증)
- **size estimate**: docs diff ~80 lines + 재측정 N=3 raw output 분석
- **priority**: **high** — main 에 false positive 보고서가 이미 머지된 상태
