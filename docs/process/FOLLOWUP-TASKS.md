...
### ~~task-OPS-002~~ — run-onprem-baseline.sh robustness 보강 ✅ **PR #57 머지**
- **출처**: PR #54 architect MED + test-engineer MED + code-reviewer MED 다수.
- **처리 PR**: PR #57 — 7 robustness 항목 + 사용자 사내 환경 호환 (`PYTHON_BIN` env override) closure:
  - (a) session DB SQL — 측정 시작 시각 `STARTED_AT` 캡쳐 + `WHERE created_at >= STARTED_AT`
  - (b) tool result error detection — JSON parse 후 `"error" in obj` key 기반 판정
  - (c) `MACRO_LOGBOT_BACKEND_CONTAINER` env override + `docker compose ps -q` 자동 검출
  - (d) run log 분리 (`run-N{1..N}.log`) + `tee` 진행상황 화면 출력
  - (e) `RATE_LIMIT_COOLDOWN` env override
  - (f) docker exec 실패 시 측정 결과 보존 (`set +e` 격리)
  - (g) N / CASES arg 정수 + format 검증
  - (h) **`PYTHON_BIN` env override** (사용자 발견) — host Python 3.14 미지원 환경 대응. PoC 스크립트는 stdlib + yaml 만 의존 → Python 3.8+ 충분. `PYTHON_BIN=python3.11` 같이 override.
  - (i) host Python pre-flight check — Python 3.8+ + pyyaml 설치 확인 후 fail-fast
- bats test 는 별도 task (task-TEST-INT-002) 로 분리. 본 PR 의 scope = script + docs 만.
