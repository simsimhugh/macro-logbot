...
- **scope**: `poc/scripts/eval/evaluate.py` 가 현재 `POST /agent/analyze` 단일 호출. spec `docs/design/02-설계문서.md` §9.4 + `docs/process/04-PoC-운영가이드.md` §6.3 는 `POST /events` → session_id polling → `GET /sessions/<id>/report` 흐름. **task-MVP-004 (session 통합) 완료 후** evaluate.py 마이그레이션, 또는 단기적으로 04-PoC-운영가이드 §6.3 endpoint 명세를 현행 MVP 와 일치하게 annotation.
...
- **scope**: 현재 `poc/scripts/eval/evaluate.py` 의 `score_1a()` 가 이전 4-channel 25%×4 의 1-A heuristic (`0.4·file + 0.3·line + 0.3·kw_ratio`) 그대로. 새 30점 기준 (도구 호출 5 + 리포트 생성 5 + file:line 10 + tool 적절 10) 에 정합되어야:
...
- **scope**:
  ...
  - (b) `sed -i` GNU/BSD 비호환 → Python 스크립트 `poc/scripts/ops/redact_onprem.py` 분리
