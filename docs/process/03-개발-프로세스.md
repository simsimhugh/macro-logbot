...
| **결정** | ✅ **APPROVE**: 위 항목 모두 충족 → (1) APPROVE summary comment 게시 (2) **봇 PAT 으로 `gh pr review --approve` 실제 review approve** (§10.5 branch protection 강제) (3) orchestrator 가 `gh pr merge --squash --delete-branch`<br>❌ **REQUEST_CHANGES**: 미달 항목 명시 → executor에게 수정 호출<br>⏸ **NEEDS_HUMAN**: 3회 재시도 후에도 미달 → label `needs-human-review` 부착, 사람 호출 |
| **출력** | summary comment + (APPROVE 시) 봇 PAT review approve + merge commit message 작성 |
| **호출 명령** | (1) `GH_TOKEN="$GH_TOKEN_BOT" gh pr review <N> --approve --body "<summary>"` (봇)<br>(2) `gh pr merge <N> --squash --delete-branch` (orchestrator) |
...
merge_action:                  # §10.5 branch protection 흐름
  step_1: verifier APPROVE summary comment 게시 (봇 PAT)
  step_2: GH_TOKEN=$GH_TOKEN_BOT gh pr review <N> --approve (봇 PAT — branch protection required review 1 충족)
  step_3: gh pr merge <N> --squash --delete-branch (orchestrator, 사용자 계정)
```

`verifier` agent는 이 YAML과 동일한 schema로 PR 상태를 평가합니다. APPROVE 결정 시 위 `merge_action` 세 단계를 순차 실행합니다.
...
README 의 "Process enforcement layer (다른 프로젝트 적용)" 절 참조.
...
| Review approve | verifier APPROVE 시 봇 PAT로 `gh pr review <N> --approve` (§10.5) |
| 자동 머지 | review approve 완료 후 orchestrator (사용자 계정) `gh pr merge <N> --squash --delete-branch` |
