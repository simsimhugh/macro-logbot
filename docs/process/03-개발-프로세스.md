...
| `pre-bash-gate.sh` | Bash hook — 우회(alias/env/체인/subshell) 차단 + caller identity 검증 | [`.claude/hooks/pre-bash-gate.sh`](../../.claude/hooks/pre-bash-gate.sh) |
| `post.sh` | review entry — caller = sub-agent 전용 + scope/verdict verify | [`.claude/skills/post-review/post.sh`](../../.claude/skills/post-review/post.sh) |
| `safe-push/run.sh` | push entry — branch 검증 + CI wait + caller = main 전용 | [`.claude/skills/safe-push/run.sh`](../../.claude/skills/safe-push/run.sh) |

> caller identity(push=main / review=sub-agent) 의 탐지 범위·전제·residual 은 각 entry 의 SKILL.md 소유.
