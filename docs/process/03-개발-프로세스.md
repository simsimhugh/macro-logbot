...
| **통과 조건** | BLOCK 0개 (WARN은 통과 — 단 verifier가 종합 판단) |
...
| **통과 조건** | CRITICAL 0개, HIGH 0개. MEDIUM은 verifier가 종합 판단 |
...
| **검토 기준** | (1) architect: BLOCK 0<br>(2) code-reviewer: CRITICAL/HIGH 0<br>(3) security-reviewer: CRITICAL/HIGH 0<br>(4) test-engineer: 3개 조건 충족<br>(5) PR이 명시한 AC가 모두 코드에 반영<br>(6) spec(요구사항·설계)과 일관 |
| **결정** | ✅ **APPROVE**: 위 6개 모두 충족 → `gh pr merge --squash --auto`<br>❌ **REQUEST_CHANGES**: 미달 항목 명시 → executor에게 수정 호출<br>⏸ **NEEDS_HUMAN**: 3회 재시도 후에도 미달 → label `needs-human-review` 부착, 사람 호출 |
...
- 같은 PR 내에서 reviewer가 BLOCK/CRITICAL/HIGH 제기 → executor 재작업
- **재시도 한도**: 3회
- 4회째 미통과 시:
...
본 문서 자체도 PR로 변경합니다 (`docs/<name>` 브랜치). 다만 본 문서 변경은 **메타 변경**이므로:
- architect agent + verifier agent만 호출 (다른 reviewer는 skip)
- 사람 1회 final review 권장 (자기 정의 문서이므로)
