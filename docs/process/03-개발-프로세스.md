...
| **메타 층 (Meta)** | 그 SDLC를 AI 서브에이전트 자동화로 운영 (AI DLC) | 본 문서가 정의 |
...
| **호출 트리거** | architect 통과 후 (§4.3 — security-reviewer · test-engineer 와 병렬 호출 가능) |
| **검토 대상** | PR로 변경된 모든 파일 라인별 |
| **검토 기준 (severity)** | **CRITICAL**: 정확성 버그, 데이터 손실, 보안 결함<br>**HIGH**: 회귀 위험, 명백한 성능 문제, 잘못된 logic<br>**MED** (Medium — 중간 심각도): 가독성·작은 버그·edge case 누락<br>**LOW**: 스타일·네이밍·minor refactor<br>**INFO**: 제안 (선택) |
| **출력** | (1) 각 발견사항: **line-level comment** (CRITICAL/HIGH는 필수, MED 이하는 선택) (2) summary general comment 1개 |
| **통과 조건** | CRITICAL 0개, HIGH 0개, **MED 0개**. MED 발견 시 무시 금지 — 본 PR scope 안이면 수정 commit, scope 밖이면 [`FOLLOWUP-TASKS.md`](FOLLOWUP-TASKS.md) queue 등록. LOW/INFO는 정보성 (차단 없음) |
...
| **모델** | Opus (보안 판단 보수적) |
| **호출 트리거** | architect 통과 후 (§4.3 — code-reviewer · test-engineer 와 병렬 호출 가능) |
| **검토 대상** | 변경 코드, 새 의존성(CVE 검색), 시크릿 패턴, 네트워크 호출 |
| **검토 기준** | **CRITICAL**: 시크릿 hardcoded, RCE 가능성, 인증 우회, 사내 데이터 외부 통신<br>**HIGH**: SQL/Command/Path injection, XSS, 인가 결함<br>**MED**: 의존성 CVE, 시큐어 코딩 미준수<br>**LOW**: 보안 권장사항 |
...
| **통과 조건** | CRITICAL 0개, HIGH 0개, **MED 0개** (위 §3.3과 동일 정책 — 본 PR scope 안 수정 또는 [`FOLLOWUP-TASKS.md`](FOLLOWUP-TASKS.md) 등록). LOW는 정보성 |
...
| **모델** | Sonnet 기본 |
| **호출 트리거** | architect 통과 후 (§4.3 — code-reviewer · security-reviewer 와 병렬 호출 가능) |
...
| **실행 명령** | `pytest tests/ -v --cov=src --cov-fail-under=80` 그리고 PoC PR이면 `python poc/scripts/eval/evaluate.py --quick` |
...
| **검토 기준** | (1) architect: BLOCK 0, WARN 처리 완료 (수정 또는 FOLLOWUP-TASKS.md 등록)<br>(2) code-reviewer: CRITICAL/HIGH/**MED** 0 (또는 follow-up PR로 분리)<br>(3) security-reviewer: CRITICAL/HIGH/**MED** 0 (또는 follow-up PR로 분리)<br>(4) test-engineer: 3개 조건 충족<br>(5) PR이 명시한 AC가 모두 코드에 반영<br>(6) spec(요구사항·설계)과 일관<br>(7) **모든 WARN/MED이 본 PR 수정 또는 [`FOLLOWUP-TASKS.md`](FOLLOWUP-TASKS.md) 등록 둘 중 하나로 처리됨** (무시 금지) |
| **결정** | ✅ **APPROVE**: 위 항목 모두 충족 → Mergify auto-merge (또는 사용자 admin bypass) 로 머지. verifier 의 raw `gh pr merge` 직접 호출 금지 (settings.deny + hook 차단).<br>❌ **REQUEST_CHANGES**: 미달 항목 명시 → executor에게 수정 호출 → 같은 reviewer 자동 재spawn (§5.1 의 N=3 retry cycle)<br>⏸ **NEEDS_HUMAN**: 3회 재시도 후에도 미달 → label `needs-human-review` 부착, 사람 호출 |
| **출력** | summary comment + (APPROVE 시) Mergify auto-merge 대기 또는 사용자 admin bypass |
| **머지 방법** | Mergify auto-merge (모든 required check + review 충족 시 자동) 또는 사용자 admin bypass. raw `gh pr merge` / `gh pr review --approve` 직접 호출은 `permissions.deny` + hook 차단. |
...
- 같은 PR 내에서 reviewer가 BLOCK/CRITICAL/HIGH/**WARN/MED(본 PR scope 안일 때)** 제기 → executor 재작업 (scope 밖 WARN/MED은 §6.8 follow-up PR 경로)
- **재시도 한도**: 3회
- 4회째 미통과 시:
...
| 재시도 트리거 | 정책 |
|---|---|
| post.sh TOCTOU exit 5 (HEAD SHA 변경) | 즉시 재호출 — HEAD 변경은 새 commit 으로 간주, 재시도 횟수 증가 안 함 |
| post.sh identity verify exit 2 | 재시도 1회, 그래도 실패 시 사람 호출 |
| reviewer API 에러 (exit 5) | 재시도 1회, 그래도 실패 시 `agent-failure` label |
...
PR #8 (feat/llm-gateway) reviewer cycle 부터 적용 — **non-meta PR 한정**. §9 메타 변경 PR 은 본 정책 적용 대상 아님 (architect+verifier 단독 흐름). 본 정책 이전 PR (#1~#7) 은 모두 직렬로 진행됨 — soft migration, 기존 PR 재진행 불필요.
...
auto_merge_conditions:
  ...
  code_reviewer:
    critical: 0
    high: 0
    medium: 0                 # MED 발견 시 본 PR scope 안 수정 또는 follow-up PR 분리
  security_reviewer:
    critical: 0
    high: 0
    medium: 0
  ...
  verifier:
    ...
    all_warns_addressed: true # WARN/MED 모두 처리 완료 (수정 OR follow-up PR 등록)
  retries:
    max: 3
    on_exhausted: "label_needs_human_review"

merge_action:                  # §5.1 task-PROCESS-002 이후
  method: Mergify auto-merge   # 모든 required check + review 충족 시 Mergify 가 자동 squash merge
  fallback: 사용자 admin bypass # Mergify 미동작 시 사용자 수동 (raw gh pr merge 직접 호출 금지)
  # raw merge 명령 금지:
  #   gh pr merge <N> --squash    → settings.deny + hook 차단
  #   gh pr review --approve      → settings.deny + hook 차단 (post-review/post.sh 경유 필수)
```

`verifier` agent는 이 YAML과 동일한 schema로 PR 상태를 평가합니다. APPROVE 결정 시 Mergify auto-merge 대기 (§5.1 — task-PROCESS-002 enforcement). raw `gh pr merge` 직접 호출 금지.

### 5.1 Process enforcement layer (task-PROCESS-002 — 2026-05-21)

§5 의 정량 기준은 docs/memory 의 **soft enforcement** — 본인 (Claude main session) 의 자의 변경 사례 반복 (PR #61 의 reviewer 재spawn skip 등). **code-level hard enforcement** 추가:

#### Layer 1 — Claude Code 설정 + hook (client-side)

- `.claude/settings.json` 의 `permissions.deny` — raw `gh pr merge` / `git push * main` / 우회 명령 (`gh api ... merge`, `git update-ref refs/heads/main` 등) enumerate
- `.claude/hooks/pre-bash-gate.sh` — Bash PreToolUse hook (deny 우회 시 safety net, `SAFE_MERGE_BYPASS=1` env 로 skill 안 호출은 통과)
- `.claude/skills/safe-merge.md` + `.claude/skills/safe-push.md` — entry skill (reviewer 5 + verifier APPROVE 검증 후 raw 명령 호출)

#### Layer 2 — Git client-side hook (보조)

- `.githooks/pre-push` + `git config core.hooksPath .githooks` — main/master 직접 push 차단

#### Layer 3 — GitHub side (server-side, 사용자 1회 setup)

- `.github/CODEOWNERS` — review 의무자 자동 request
- `.github/pull_request_template.md` — PR description format 강제 보조
- GitHub branch protection rule: force-push 금지, required PR review, required status check

#### 머지 flow (본인 사용)

본인이 머지 시 raw `gh pr merge` 직접 호출 **금지** (settings.deny + hook 차단).

머지 방법:
1. **Mergify auto-merge** — 모든 required check + PR review 충족 시 Mergify 가 자동 squash merge. verifier APPROVE 후 자동으로 진행.
2. **사용자 admin bypass** — Mergify 미동작 시 사용자가 GitHub UI 또는 admin 권한으로 직접 머지.

raw `gh pr merge` / `git push main` 은 settings.deny + hook 이 항상 차단.

#### 다른 프로젝트 적용

README 의 "Process enforcement layer (다른 프로젝트 적용)" 절 참조. `poc/scripts/ops/setup-enforcement.sh` 로 1회 setup 자동화.

### 5.1.1 머지 flowchart

```mermaid
flowchart TD
    Start([머지 시도]) --> RawCmd{명령 type?}
    RawCmd -- "raw gh pr merge / git push main / 우회" --> Block1[Layer 1: settings.deny BLOCK]
    Block1 --> SR1[stderr: Mergify auto-merge 또는 사용자 admin bypass 사용 알림]
    RawCmd -- "verifier APPROVE 후 대기" --> Mergify[Mergify auto-merge]
    Mergify --> GH[Layer 3: GitHub branch protection]
    GH --> Done([머지 완료])
    RawCmd -- "Mergify 미동작" --> AdminBypass[사용자 admin bypass]
    AdminBypass --> GH
```

### 5.1.2 emergency hotfix path (사용자 manual 만)

본인 (Claude main session) 은 emergency 회피 path 사용 **불가** — `.claude/settings.json` 의 `permissions.deny` 가 `git push --no-verify` 차단.

**사용자 (human) manual 시도**:
```bash
# 사용자가 직접 git CLI 사용 (Claude 우회):
git push --no-verify origin main
```

본 사용 시:
- 사용자가 명시적 의도 (긴급)
- 사후 PR 작성 + reviewer review 의무 (post-hoc compliance)
- task-EMERGENCY-XXX 로 follow-up

## 6. 흔적 가이드라인 (Showing AI DLC)
...
---
_Auto-generated by macro-logbot AI DLC pilot. Session: .omc/sessions/<id>_
```
...
> **Severity: <CRITICAL | HIGH | MED | LOW>** — file:line + 한 줄 요약
...
| WARN / MED | `> [!WARNING]` | 권장 수정 (BLOCK 아님, §6.8 처리 의무) |
| PASS | `> [!TIP]` | 머지 기준 충족 |
...
### 6.8 WARN/MED 무시 금지 + Follow-up PR 의무

reviewer agent가 발견한 **WARN** (architect) / **MED** (code-reviewer · security-reviewer) finding은 무시 금지. 두 가지 경로:
...
**원칙**: 어떤 WARN/MED도 "무시"로 끝나지 않는다. 본 PR 처리하거나 follow-up PR로 정리.
...
# §4.3 호출 모드: architect 단일 → fix → CR/Sec/Test 병렬 → verifier 단일
...
def handle_task(task_spec):
    ...
    # 2. architect 단일 호출 (§4.3 step 1)
    arch_review = call_reviewer_with_retry("architect", pr_number, max_retries=3)
    if arch_review.exhausted:
        gh_pr_label(pr_number, "needs-human-review"); return

    # 2b. architect WARN scope-out follow-up 등록 (§6.8)
    register_followup_tasks(arch_review.scope_out_findings)

    # 3. CR / Sec / Test 병렬 호출 (§4.3 step 3 — 평가축 직교)
    reviews = asyncio.gather(*[
        call_reviewer_with_retry(r, pr_number, max_retries=3)
        for r in ("code-reviewer", "security-reviewer", "test-engineer")
    ])
    if any(r.exhausted for r in reviews):
        gh_pr_label(pr_number, "needs-human-review"); return

    # 4. verifier 단일 호출 (§4.3 step 4)
    verdict = Agent(subagent_type="oh-my-claudecode:verifier",
                    prompt=f"Apply §5 auto_merge_conditions to PR #{pr_number}")
    if verdict == "APPROVE":
        # §10.5 merge_action step 1: APPROVE summary comment (봇)
        gh_pr_comment(pr_number, verdict.summary)
        # §10.5 merge_action step 2: 봇 PAT 실제 review approve (branch protection 강제)
        gh_pr_review_approve(pr_number, bot_token=GH_TOKEN_BOT, body=verdict.summary)
        # §10.5 merge_action step 3: orchestrator (사용자) 가 머지 — branch protection 통과
        gh_pr_merge(pr_number, "--squash --delete-branch")


def call_reviewer_with_retry(reviewer, pr_number, max_retries):
    # §4.2 재시도: 같은 reviewer 재호출은 직렬 (§4.3 단일 호출 강제 케이스 #2)
    for _ in range(max_retries):
        review = Agent(subagent_type=f"oh-my-claudecode:{reviewer}",
                      prompt=f"Review PR #{pr_number} per §3.X")
        gh_pr_comment(pr_number, format_comment(reviewer, review))  # bot PAT
        if not review.has_blocker():
            return review
        Agent(subagent_type="oh-my-claudecode:executor",
              prompt=f"Address {reviewer} comments on PR #{pr_number}")
    return ReviewResult(exhausted=True)
```
...
| Review approve | verifier APPROVE 후 Mergify auto-merge 대기 (§5.1). raw `gh pr review --approve` 직접 호출 금지 (settings.deny + hook 차단) |
| 자동 머지 | Mergify auto-merge (모든 required check + review 충족 시 자동 squash merge). raw `gh pr merge` 는 §5.1 의 settings.deny + hook 차단 |
...
- **머지 직전 봇 review approve 1개 존재** (§10.5) — `gh pr view <N> --json reviews` 로 `author.login=simsimhugh-bot` `state=APPROVED` 확인
- BLOCK/CRITICAL/HIGH 발생 PR은 재작업 commit이 comment 다음 시각으로 존재

### 10.5 Branch Protection (Required Reviews 1)

main 브랜치는 다음 GitHub branch protection rule 로 보호:

| 항목 | 설정 | 비고 |
|---|---|---|
| Required pull request reviews | **1 approval** | verifier 봇 (simsimhugh-bot) 가 충족 |
| Dismiss stale approvals on push | **활성화** | 새 commit push 시 이전 approve 자동 무효화 → §6.5 시간 순서 정직 보존, fix commit 후 verifier 재호출 강제 |
| Require status checks | 비활성화 | 현 Stage A — GitHub Actions 미채택 (§10.2). 향후 Stage B 진입 시 추가 |
| Enforce admins | 비활성화 | 사용자 admin override 가능 (긴급 시) |
| Restrict push | 비활성화 | 직접 push 차단 안 함 (orchestrator 가 PR 외 패턴 사용 가능성 보존) |

#### Approve 주체

**verifier agent 만** review approve 권한 행사:

- `GH_TOKEN="$GH_TOKEN_BOT" gh pr review <N> --approve --body "<summary>"`
- 봇 계정 (simsimhugh-bot) 이 PR author (simsimhugh) 와 별개라 self-approve 위반 없음
- §5 `auto_merge_conditions` 모두 충족 시에만 호출

#### 자동 머지 흐름 (§5 `merge_action` 의 GitHub 측 강제)

1. verifier `APPROVE` summary comment 게시 (봇 PAT)
2. verifier `gh pr review --approve` 실제 review approve (봇 PAT) — **branch protection 의 required review 1 충족**
3. orchestrator (사용자 계정) `gh pr merge <N> --squash --delete-branch` — branch protection 통과

만약 step 2 가 누락되면 step 3 가 **403 / merge blocked** 로 자동 실패 → 정직성 GitHub 측에서 강제. orchestrator 가 인간 실수로 머지 시도해도 막힘.

#### 활성화 방법 (admin 작업, 사용자 직접 실행)

```bash
gh api -X PUT /repos/simsimhugh/macro-logbot/branches/main/protection \
  --input - <<'JSON'
{
...
또는 GitHub Settings → Branches → Add rule (`main`).

#### 메타 PR 흐름 (§9) 과의 호환

§9 메타 PR (architect + verifier 단독) 도 동일 적용. verifier APPROVE → 봇 review approve → 머지. CR/Sec/Test skip 은 변하지 않음.

#### 적용 시점

본 정책 머지 후 **다음 PR 부터** branch protection rule 활성화 (사용자 admin 작업). 이전 PR (#1~#9) 은 branch protection 없이 머지된 상태 — soft migration, retrospective 비활성화 불요.
