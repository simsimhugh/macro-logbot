...
| **결정** | ✅ **APPROVE**: 위 항목 모두 충족 → `/safe-merge <PR-NUM>` skill 호출 (§5.1 task-PROCESS-002 이후 단일화 — 내부에 bot PAT approve + squash merge)<br>❌ **REQUEST_CHANGES**: 미달 항목 명시 → executor에게 수정 호출 → 같은 reviewer 자동 재spawn (§5.1 의 N=3 retry cycle)<br>⏸ **NEEDS_HUMAN**: 3회 재시도 후에도 미달 → label `needs-human-review` 부착, 사람 호출 |
| **출력** | summary comment + (APPROVE 시) `/safe-merge` 호출 + merge commit message 작성 |
| **호출 명령** | `/safe-merge <PR-NUM>` (§5.1 task-PROCESS-002 이후) — 본 skill 이 check.sh + bot PAT approve + raw merge 통합. raw `gh pr merge` / `gh pr review --approve` 직접 호출은 `permissions.deny` + hook 차단. |
...
merge_action:                  # §5.1 task-PROCESS-002 이후 — /safe-merge 단일화
  step_1: /safe-merge <PR-NUM>  # check.sh 가 위 정량 기준 자동 검증 + N=3 retry + bot PAT approve + raw merge
  # 옛 manual sequence (§5.1 이전):
  #   step_a: verifier APPROVE summary comment 게시 (봇 PAT)
  #   step_b: GH_TOKEN=$GH_TOKEN_BOT gh pr review <N> --approve (봇 PAT)
  #   step_c: gh pr merge <N> --squash --delete-branch (orchestrator)
  # → task-PROCESS-002 이후 /safe-merge skill 의 check.sh 안 통합 (code-level enforce).
```

`verifier` agent는 이 YAML과 동일한 schema로 PR 상태를 평가합니다. APPROVE 결정 시 `/safe-merge <PR-NUM>` skill 호출 (§5.1 — task-PROCESS-002 enforcement).
...
본인이 머지 시 raw `gh pr merge` 직접 호출 **금지** (settings.deny + hook 차단). 다음 skill 만 entry:
...
본 skill 가 자동 검증:
1. PR comment 5 이상 (4 reviewer + 1 verifier)
2. 각 reviewer 의 최근 verdict (APPROVE / PASS)
3. **REQUEST CHANGES 후 fix 시 같은 reviewer 의 재approve** ← PR #61 의 자의 위반 사례 자동 차단
4. PR mergeable 상태

통과 시 `SAFE_MERGE_BYPASS=1 gh pr merge <PR-NUM> --squash` 호출 (skill 안 내부, 사용자 manual 호출 금지).
...
```mermaid
flowchart TD
    Start([본인 머지 시도]) --> RawCmd{명령 type?}
    RawCmd -- "raw gh pr merge / git push main / 우회" --> Block1[Layer 1: settings.deny BLOCK]
    Block1 --> SR1[stderr: /safe-merge 사용 알림]
    SR1 --> Skill[/safe-merge PR-NUM/]
    RawCmd -- "/safe-merge" --> Skill
    Skill --> CheckSh[".claude/skills/safe-merge/check.sh"]
    CheckSh --> Verdict{exit code}
    Verdict -- "0 PASS" --> BypassMerge["SAFE_MERGE_BYPASS=1<br/>gh pr merge --squash"]
    BypassMerge --> GH[Layer 3: GitHub branch protection]
    GH --> Done([머지 완료])
    Verdict -- "1 FAIL" --> Retry{N < 3?}
    Retry -- "Yes" --> RespawnRev[fail reviewer 재spawn]
    RespawnRev --> PostComment[reviewer comment 게시 bot PAT]
    PostComment --> CheckSh
    Retry -- "No 3+ retry" --> Manual[사용자 manual 보고]
    Verdict -- "2 argument" --> Manual
```
...
| Review approve | verifier APPROVE 시 `/safe-merge <PR-NUM>` 안 자동 bot PAT approve (§5.1 — task-PROCESS-002 이후 단일화) |
| 자동 머지 | `/safe-merge <PR-NUM>` 안 자동 squash merge (옛 manual `gh pr merge` 는 §5.1 의 settings.deny 차단) |
