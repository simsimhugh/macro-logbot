...
각 fix sub-agent 는 **working tree 만 편집하고 개별 commit 하지 않는다**. commit 통합·push 규칙은 [`fix-review/SKILL.md`](../../.claude/skills/fix-review/SKILL.md) 참조.

**verify** — fix 수렴 후 **단일 verifier sub-agent** 가 변경 전체를 한 번에 검증하고 `PASS`/`FAIL` 로 main 에 보고만 한다 (push 금지). verifier 구성·검증 의무·FAIL 기준은 [`verify-fix/SKILL.md`](../../.claude/skills/verify-fix/SKILL.md) 참조.
