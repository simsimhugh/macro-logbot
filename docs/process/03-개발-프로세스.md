...
sequenceDiagram
    ...
    R->>M: REQUEST_CHANGES
    M->>F: fix 위임 (reviewer별·순차·모델고정)
    F->>M: fix 완료 (commit 1개)
    M->>V: verify 위임
    V->>M: PASS / FAIL 보고
    Note over M: FAIL → 다시 fix / PASS → push
...
모든 fix(그 결과의 lint/type/format fix 포함)는 **commit 1 개로 통합** (last review SHA 이후 HEAD 까지 commit 정확 1 개 — 분리됐으면 `git commit --amend` 또는 `git reset --soft <last-review-sha> && git commit -F file` 로 squash). verify `PASS` 후 main 이 `bash run.sh <BRANCH> --force-with-lease` 로 re-push.

**verify** — fix 수렴 후 **단일 verifier sub-agent** (agent type `verifier`) 가 **변경 전체를 한 agent 가 한 번에** 검증 (= 전체 통합 검증). reviewer 별로 안 쪼갬 — 한 도메인의 fix 가 다른 도메인의 fix 를 깨뜨리는 regression 은 전체를 봐야 잡히기 때문. 결과를 `PASS`/`FAIL` 로 **main 에 보고만** 함 (push 금지).

**push** — verify 결과별 main 의 행동:

- `FAIL` → main 이 재-fix 지시 (fix→verify 재실행 — 이 루프엔 별도 횟수 제한 없음. §5.4 의 3 cycle 한도는 reviewer cycle 기준)
...
같은 finding 에 대한 fix 재시도는 **최대 3 cycle**. 3 cycle 후에도 reviewer 가 같은 finding 으로 REQUEST_CHANGES 시:

- 본 PR 에 `needs-human-review` label 부착
- 사용자 개입 (orchestrator 자율 진행 중단)
