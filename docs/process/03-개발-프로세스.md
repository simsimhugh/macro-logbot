...
각 fix sub-agent 는 **working tree 만 편집하고 개별 commit 하지 않는다** — 순차 fix 가 모두 끝난 뒤 **main 이 1 회 commit 으로 통합**하는 것을 **권장**한다 (cycle 당 새 commit 1 개 — 작성 위생, 그 결과의 lint/type/format fix 포함). verify `PASS` 후 main 이 `bash run.sh <BRANCH>` 로 push 한다.

- **이미 push 된 commit 은 force push 금지** — amend / `reset --soft <sha> && git commit` 으로 직전 commit 에 수렴시키지 않는다. fix cycle 이 의도치 않게 여러 commit 을 push 해도 괜찮다 (1 commit 은 권장일 뿐 강제 아님).
- PR 중간 commit 수는 main 히스토리에 영향 없다: 새 commit push 는 Mergify 의 approve dismiss → 재리뷰를 트리거하고, 머지 시점에 어차피 squash merge (§6) 되기 때문.
