...
verdict 는 finding severity 기반 role-specific 자동 산출 (agent 자율 결정 금지). **정책 표·계산 규칙의 단일 진실** = [`post-review/SKILL.md`](../../.claude/skills/post-review/SKILL.md) § "verdict 자동 결정" + `post.sh` `expected_verdict()`.
...
**fix** — REQUEST_CHANGES 낸 **reviewer 별 전용 fix sub-agent** (agent type `executor`) 를 **단일 worktree 에서 순차** 호출. 모델은 reviewer 도메인별 정적 고정 (런타임 escalate 판단 없음) — reviewer↔fix 모델 매핑은 [`fix-review/SKILL.md`](../../.claude/skills/fix-review/SKILL.md) 가 단일 진실.
...
각 fix sub-agent 는 **working tree 만 편집하고 개별 commit 하지 않는다**. commit 통합·push 규칙(권장·강제 여부 포함)은 [`fix-review/SKILL.md`](../../.claude/skills/fix-review/SKILL.md) 가 단일 진실.

**verify** — fix 수렴 후 **단일 verifier sub-agent** 가 변경 전체를 한 번에 검증하고 `PASS`/`FAIL` 로 main 에 보고만 한다 (push 금지). verifier 구성·검증 의무·FAIL 기준의 단일 진실 = [`verify-fix/SKILL.md`](../../.claude/skills/verify-fix/SKILL.md).

**push** — verify 결과별 main 의 행동:

- `FAIL` → main 이 재-fix 지시 (fix→verify 재실행). 재시도 한도는 §5.4 (정의: 스킬).
...
두 루프(outer reviewer cycle · inner fix→verify) 모두 한도 초과 시 `needs-human-review` label + 사용자 개입 (orchestrator 자율 진행 중단). **각 한도 값·초과 조건의 단일 진실**은 owning 스킬:
...
| fix commit 통합·push 규칙 | [`fix-review/SKILL.md`](../../.claude/skills/fix-review/SKILL.md) | §5.2 참조 |
| verifier 구성·FAIL 기준·inner 재시도 한도 | [`verify-fix/SKILL.md`](../../.claude/skills/verify-fix/SKILL.md) | §5.2·§5.4 참조 |
| reviewer cycle(outer) 재시도 한도·reviewer spawn 의무 | [`safe-push/SKILL.md`](../../.claude/skills/safe-push/SKILL.md) | §5.4 참조 |
