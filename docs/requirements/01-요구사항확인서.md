# 요구사항 확인서: macro-logbot

## 1. 문서 정보
| 항목 | 내용 |
|---|---|
| 프로젝트명 | macro-logbot |
| 단계 | Stage 1/3 — 요구사항 확인서 |
| 버전 | 0.4 |
| 작성일 | 2026-05-15 |
...
| NFR-2 | 보안 | 분석 대상 코드·로그는 사내 환경 외부로 유출 금지 (네트워크 감사로 검증) |
...
- [ ] AC-5: 검증셋 기준 자율 해결률을 자동 측정하는 평가 스크립트가 존재한다
...
```mermaid
flowchart LR
    R1["Stage 1<br/>요구사항 확인서<br/>(현재)"] --> R2["Stage 2<br/>설계 문서"]
    R2 --> R3["Stage 3<br/>구현"]
    R3 --> R4["코드위키<br/>운영 문서"]
```
