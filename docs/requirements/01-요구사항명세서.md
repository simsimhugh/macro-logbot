...
- [ ] AC-5: 검증셋 기준 자율 해결률을 자동 측정하는 평가 스크립트가 존재한다. 채점은 4-channel 가중 합산 — `score = 0.25·(1-A 위치) + 0.25·(1-B 원인) + 0.25·(2-A follow-up 도구) + 0.25·(2-B fix_hint)`, 결정론적 재현을 위해 `temperature=0`, `seed=42` 적용 (설계문서 §10.1, PoC 가이드 §6.3)
