...
| `MACRO_LOGBOT_LLM_BASE_URL` | 미설정 (Gemini/OpenAI 등 사외 LLM) | `https://<사내-llm-endpoint>` | ⏳ task-LG-002 후속 (LLM endpoint, mirror 와 별개) |

사외→사내 전환 비용: `.env` **3 줄 (image/pip mirror)** + task-LG-002 머지 시 LLM endpoint 1 줄. 추가 스크립트/profile/manifest 분리 X — 오버엔지니어링 회피.
