...
| **메타 층 (Meta)** | 그 SDLC를 AI 서브에이전트 자동화로 점진 전환 (AI-driven SDLC) — Stage 3부터 적극 적용 |

따라서 macro-logbot은 **AI 시스템**이자 동시에 **AI-driven SDLC의 실증 사례**다. process 문서가 일관되게 동작하면 향후 사내 다른 AI 프로젝트에도 재사용 가능한 자산이 된다.
...
`docker compose up -d` 명령은 사외 PoC / 사내 운영 동일. 차이는 4 env 변수만:

| env | 사외 PoC (default) | 사내 운영 |
|---|---|---|
| `BASE_IMAGE` | `python:3.14-slim` | `<사내-registry>/python:3.14-slim` |
| `PIP_INDEX_URL` | `https://pypi.org/simple` | `https://<사내-pypi>/simple` |
| `OPEN_WEBUI_IMAGE` | `ghcr.io/open-webui/open-webui:main` | `<사내-registry>/open-webui:main` |
| `MACRO_LOGBOT_LLM_BASE_URL` | 미설정 (Gemini/OpenAI 등 사외 LLM) | `https://<사내-llm-endpoint>` (task-LG-002 후속) |

사외→사내 전환 비용: `.env` 4 줄 변경. 추가 스크립트/profile/manifest 분리 X — 오버엔지니어링 회피.
