...
| 선행 문서 | `docs/requirements/01-요구사항확인서.md` (Stage 1 v0.4) |
...
- macro-logbot service를 **OpenAI 호환 backend**로 연결 (`OPENAI_API_BASE_URL=http://macro-logbot:8000/v1`)
...
services:
  ...
  open-webui:
    ...
    ports: ["3000:8080"]
    environment:
      OPENAI_API_BASE_URL: http://macro-logbot:8000/v1
```
...
sequenceDiagram
    ...
    participant TS as Tool System<br/>(MCP, 8 tools)
...
```
1. Baseline 구현 (LangGraph + LiteLLM + MCP 8 tools)
...
| 모드 | 동작 | 측정 의미 |
|---|---|---|
| `isolated` | 매 case마다 KB 초기화. retrieval 비활성 | **Baseline** — 시스템 + LLM 순수 능력 |
