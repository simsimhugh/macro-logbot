...
> **구현 상태** (2026-05-19, PR #18 머지): ✅ `src/macro_logbot/agent/core.py` 가 LangGraph `StateGraph` 로 구현 — 핵심 3 노드 (`llm_call` / `route` / `execute_tools`) + `END`. 본 § 의 `intake` / `crystallize_report` / `followup` 3 노드는 후속 (task-MVP-001-x).
