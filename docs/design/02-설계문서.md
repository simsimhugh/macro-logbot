...
flowchart LR
    subgraph 사외환경
        ...
        FREE["무료 LLM 4종<br/>Gemini Flash Lite · gpt-4o<br/>Claude Haiku · Groq Llama"]
    end
...
>
> **PR #26 (task-MVP-004-x) 갱신**: `session_id: str | None` + `event_id: str | None` 필드 추가 완료. `pending_tool_calls` / `tool_results` 는 spec 잠정 필드 — LangGraph node 함수 시그니처와 중복이므로 구현 미정.
...
class AgentState(TypedDict):
    ...
    report: Report | None
    session_id: str | None   # endpoint 가 채워서 전달 (spec line 134)
    event_id: str | None     # Log Event 1:N 관계 키 (spec line 135, MVP 단순화)
    _model: str | None
    _generation_kwargs: dict[str, object]
    _gateway: LLMGateway
```

> **잠정 필드 (미구현)**: `event: LogEvent` (spec line 135) → `event_id: str | None` 로 MVP 단순화. `pending_tool_calls: list[ToolCall]` / `tool_results: list[ToolResult]` (spec line 137-138) → LangGraph node 함수 시그니처와 중복, graph state 로 충분. task-MVP-004-x 잔여.
...
- `crystallize_report` — 최종 답변을 구조화된 리포트로 변환 (PR #46 강화 — 아래 참조)
- `followup` — 사용자 추가 메시지에 대해 loop 재진입

**`crystallize_report` 노드 — structured JSON 강화** (PR #46):
1. LiteLLM `response_format={"type":"json_object"}` 로 structured JSON 추출 시도
2. OSS 모델이 `response_format` 미지원 시 → prompt-only fallback (JSON 출력 지시 프롬프트)
3. JSON 파싱 실패 시 retry 1회 (corrective prompt — "앞 응답을 유효한 JSON 으로 재출력")
4. traceback fallback: `location` 필드가 `None` 이면 stderr 마지막 frame 에서 file/line 자동 추출
...
**컨텍스트 truncation** (PR #45):
- 환경변수 `MACRO_LOGBOT_MODEL_CONTEXT_LIMIT` (default `16384`) 으로 토큰 상한 설정
- messages 누적 토큰이 상한의 **80% watermark** 초과 시 → **70% target** 까지 축소
- 제거 단위: `assistant(tool_calls)` + 해당 `tool` 응답 group 을 가장 오래된 것부터 pair 단위로 pop (atomic — tool_calls 만 남거나 tool 응답만 남는 불완전 상태 방지)
- system prompt · 최초 intake message · 최신 N 턴은 항상 보존
...
> **구현 상태** (2026-05-19, PR #20 머지): ✅ SQLite 저장소 (`SQLiteSessionStore`) + `SessionStore` Protocol 도입 — `src/macro_logbot/session/store.py`. messages 만 직렬화 (단일 table). `tool_history` / `follow_up_messages` / `report` 컬럼 확장은 후속 PR (task-MVP-002-x).
> **PR #24 머지** (2026-05-19): ✅ endpoint session_id 통합 완료 (task-MVP-004) — `/agent/analyze` 가 `session_id` 받아 `SQLiteSessionStore` 로 messages 누적, 응답에 `session_id` 반환. `SQLiteSessionStore.update` 가 upsert 의미로 통일 (InMemory 와 일관성).
...
> **PR #24 머지** (2026-05-19): ✅ KB auto-archive 통합 완료 (task-KB-002) — `env MACRO_LOGBOT_KB_AUTO_ARCHIVE=true` 활성화 시 `/agent/analyze` 가 분석 완료 후 `SQLiteKBStore.add(ArchivedCase)` 자동 호출. `source="poc"`, `error_signature=root_cause[:80]`, `location` 없으면 placeholder `Location(file="unknown", line=1)` 사용.
...
- 모델 식별자 (예: `claude-opus-4-7`, `gemini/gemini-2.5-flash-lite`, `openai/gpt-4o`, `groq/llama-3.1-70b-versatile`) → 적절한 provider/API 형식으로 라우팅
...
| 모델 | LiteLLM 식별자 | 환경변수 |
|---|---|---|
| Google Gemini Flash Lite | `gemini/gemini-2.5-flash-lite` | `GEMINI_API_KEY` |
...
#### Claude Code judge — 70점 (의미적 채점, PoC 운영 가이드 §7.3 흐름)
...
> 이전 4-channel 25%×4 방식 대체. 사용자 통찰: "도구 동작은 당연한 기능, 핵심은 분석을 하냐 못하냐". `evaluate.py` 의 1-A heuristic 은 자동 30점 부분만 측정. Claude judge 는 별도 단계 (수동 또는 후속 자동화).

**case별 종합 분류** (100점 만점 기준):
- `full`: 85점 이상
- `partial`: 45~84점
- `fail`: 44점 이하
...
└── scripts/
    ...
    │   └── setup-enforcement.sh       # GitHub repo enforcement
    └── demo/                          # 데모 CLI
...
**평가 명령**:
```bash
python poc/scripts/eval/evaluate.py --models all --cases all --kb-mode isolated
python poc/scripts/eval/evaluate.py --models all --cases all --kb-mode cumulative
python poc/scripts/eval/evaluate.py --models all --cases all --kb-mode pre-seeded
```
...
### 12.4 Workspace 경로 격리 정책 (PR #43)

Tool System 의 파일 접근은 `_safe_resolve` 를 통해 4-layer 보안 검증을 거친다:

| Layer | 검증 내용 |
|---|---|
| 1. literal prefix | 해석된 절대 경로가 허용 workdir prefix 로 시작하는지 확인 |
| 2. Symlink 검사 (stat-time) | `Path.is_symlink()` + parent components 검사로 symlink escape 1차 차단. **caveat**: 검사 시점과 실제 open 사이 TOCTOU race 잔존 — 진정한 `O_NOFOLLOW` open 적용은 `task-TOOL-002` follow-up |
| 3. secret blocklist | `.env`, `*.key`, `id_rsa` 등 비밀 파일 패턴 거부 |
| 4. env=poc gate | `MACRO_LOGBOT_ENV=poc` 활성화 시에만 `/tmp/poc-cases` 경로 허용 |

**환경별 동작**:
- `MACRO_LOGBOT_ENV=poc`: Tool System 이 `MACRO_LOGBOT_POC_WORKSPACE_ALLOWED` (default `/tmp/poc-cases`) 경로 접근 허용
- `MACRO_LOGBOT_ENV=production` 또는 미설정: **fail-closed** — cwd 외부 경로 접근 거부. PoC env 변수(`MACRO_LOGBOT_POC_WORKSPACE_ALLOWED`) 설정되어 있어도 무시

> **원칙**: production 에서 `/tmp/poc-cases` 마운트 자체를 제거하고 `MACRO_LOGBOT_ENV=production` 강제 설정. 두 겹 방어로 사내 운영 시 PoC workdir 노출 불가.

**env 변수 관계**: `MACRO_LOGBOT_POC_CASES_ROOT` (evaluate.py 호스트 측 inject workdir 생성 위치, default `/tmp/poc-cases`) 와 `MACRO_LOGBOT_POC_WORKSPACE_ALLOWED` (backend tool 의 prefix allowlist, default `/tmp/poc-cases`) 는 default 가 일치. dev override 시 둘 다 같이 변경 — 한쪽만 바꾸면 측정 시 tool 호출 즉시 fail (self-evident).
...
- `docs/process/03-개발-프로세스.md` — Mechanical/Semantic 분업 + 4 AI reviewer + Mergify auto-merge (AI DLC v2)
...
| 1.2 | 2026-05-20 | sprint #42~#46 반영: (1) §5.2 context truncation 추가 — `MACRO_LOGBOT_MODEL_CONTEXT_LIMIT` env, 80% watermark / 70% target, tool_calls+tool 응답 group pop (PR #45) (2) §5.2 `crystallize_report` structured JSON 강화 — `response_format` 시도, OSS fallback, retry 1회, traceback fallback (PR #46) (3) §12.4 workspace 경로 격리 정책 신설 — `_safe_resolve` 4-layer, env=poc gate, production fail-closed (PR #43) | macro-logbot 프로젝트팀 |
| 1.3 | 2026-05-20 | §10.1 채점 방식 전면 개정 — 4-channel 25%×4 → total 100 = 자동 30 (evaluate.py heuristic: 도구 호출 5 + 리포트 생성 5 + file:line 10 + tool 적절 10) + Claude judge 70 (root_cause 정확성 상40/중20/하5 + fix_hint 구체성 상30/중15/하5). 자동/judge 책임 분리 명확화. 사용자 통찰: "도구 동작은 당연한 기능, 핵심은 분석 능력" | macro-logbot 프로젝트팀 |
