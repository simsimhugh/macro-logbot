...
_ANALYZE_SYSTEM_PROMPT = (
    ...
    "## 분석 절차 (반드시 순서대로)\n"
    "1. traceback 의 마지막 frame 의 file:line 을 확인하세요.\n"
    "2. **read_file** 로 해당 line + 주변 ±10 줄을 직접 읽으세요.\n"
    "3. **grep_codebase** 로 관련 변수/함수 정의 위치를 확인하세요.\n"
    "4. **read_file** 로 정의 위치도 읽어 — 변수 초기화 / 함수 동작을 확인하세요.\n"
    "5. semantic 오류 가능성을 확인하세요:\n"
    "   - **infinite loop**: while True / for + 종료 조건 누락 확인\n"
    "   - **wrong assignment**: 좌변 우변 의미 (insert vs override) 확인\n"
    "   - **off-by-one**: range(len+1), len-1, [-1:] 등 경계 확인\n"
    "6. 최소 2 회 다른 tool 호출 후 결론을 내리세요. "
    "한 번만 보고 추측하는 것은 허용되지 않습니다.\n\n"
    "## 답안 형식 (반드시 준수)\n\n"
    "### root_cause\n"
    "- traceback 의 error class 를 명시하세요 (e.g., AttributeError, IndexError).\n"
    "- **코드 read 후 직접 확인한 사실** 만 root_cause 로 작성하세요. "
    "추측 (-가 None 일 수도 있다) 은 금지입니다.\n"
    ...
    "절대 root_cause 로 채택하지 마세요:\n"
    "- ALSA lib confmisc.c / conf.c / pcm.c — 사운드 카드 무관\n"
    "- pygame init / pygame.error — headless 환경 정상\n"
    "- GPU driver / OpenGL warning — 분석 무관\n"
    "- locale warning (en_US.UTF-8 not found) — 분석 무관\n"
