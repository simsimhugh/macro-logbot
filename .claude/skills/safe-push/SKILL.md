...
> defense-in-depth layer 이며 — command substitution·백틱·here-string·`bash -c` 내부 identity 등은
> 정적분석 한계로 미탐지(residual) — 본질 경계는 server-side(branch protection + Mergify)입니다.
