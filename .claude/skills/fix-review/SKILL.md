...
- **fix sub-agent 는 commit 안 함**: working tree 만 편집. 순차 fix 가 모두 끝난 뒤 **main 이 1 회 commit 으로 통합**하는 것을 권장 (cycle 당 새 commit 1 개 — 작성 위생, 강제 아님). 단 **이미 push 된 commit 은 force push 금지** — amend/squash 로 수렴시키지 말고 새 commit 으로 push, 최종 squash 는 Mergify (§6). 상세는 [`docs/process/03-개발-프로세스.md`](../../../docs/process/03-개발-프로세스.md) §5.2 가 단일 진실.
