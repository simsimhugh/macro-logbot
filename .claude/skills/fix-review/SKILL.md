...
**reviewer↔fix 모델 매핑** — 이 skill 이 단일 진실 (03 §5.2 는 여기를 참조):
...
- **fix sub-agent 는 commit 안 함**: working tree 만 편집. commit·push 규칙은 이 skill 이 단일 진실 (03 §5.2 는 참조): 순차 fix 가 모두 끝난 뒤 **main 이 1 회 commit 으로 통합**하는 것을 권장 (cycle 당 새 commit 1 개 — 작성 위생, 강제 아님). verify `PASS` 후 main 이 `bash run.sh <BRANCH>` 로 push.
